...
Per tick, in order:
  ...
  3. assemble the 105-D observation (G1: 18 + 3*29);
  4. run the ONNX actor -> raw_action[29];
  5. form the APPLIED action. The G1 has no passive neck (HEAD_INDICES is empty),
     so the applied action equals the raw output; it is fed back as the next
     last_action — matching training. (This step is the general passive-column
     handling shared with the A3 runner; for the G1 it is a no-op.)
  6. pass the applied action through the shared ActionAdapter -> 29 joint targets;
...
tasks, or reference playback here -- a single continuous 105-D control path.
"""
