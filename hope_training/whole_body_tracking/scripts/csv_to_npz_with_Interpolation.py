...
# Pre-defined configs
##
# Export from the exact robot HOPE trains on (G1_CFG) so the logged body world-poses come from the
# same USD the tracking task uses. Replay is kinematic (sim.render(), no physics), so the old
# G1_CYLINDER_CFG collision geometry was irrelevant.
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene, joint_names: list[str]):
    ...
    while simulation_app.is_running():
        ...
        if reset_flag and not file_saved:
            ...
            # Store the articulation's ground-truth ordering so downstream consumers can map by
            # name (e.g. the HOPE 14-tracked-body converter). Additive: does not change the raw
            # 30-body arrays the deploy pipeline reads.
